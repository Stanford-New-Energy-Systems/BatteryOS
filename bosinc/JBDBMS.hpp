...
// struct PythonSerialWrapper {
//     boost::python::object serial_module;
//     boost::python::object serial_obj;
//     PythonSerialWrapper(const std::string &devaddr, int baudrate = 9600) {
//         serial_module = boost::python::import("serial");
//         serial_obj = serial_module.attr("Serial")();
//         serial_obj.attr("baudrate") = 9600;
//     }
...
public:
    ...
    // note: we are not overloading the << operator... 
    static std::ostream & print_basic_state_to_stream(std::ostream &out, const State &s) {
        out << "State = {\n";
    ...
    // static std::tuple<bool, uint8_t, std::vector<uint8_t>> extract_received_data(const std::vector<uint8_t> &data) {
...
protected: 
    BatteryStatus refresh() override {
        ...
        this->timestamp = Util::get_system_time();
        return status;
    }
    bool check_staleness_and_refresh() {
        auto now = Util::get_system_time();
